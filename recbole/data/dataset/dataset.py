...
class Dataset(torch.utils.data.Dataset):
    ...
    def split_by_ratio(self, ratios, group_by=None):
        ...
        # n_flip = int(self.config['contaminate_prop'] * len(next_df[0]))
        # flip_inds = np.random.randint(low=0, high=len(next_df[0]), size=n_flip)
        # next_df[0].label[flip_inds] = (~next_df[0].label[flip_inds].bool()).float()
        # if self.config['eval_type'] == EvaluatorType.RANKING:
        #     if self.config['contaminate']:
        #         exp_inds = next_df[-2].explicit == 1
        #         pos_inds = next_df[-2].label == 1
        #         inds = torch.logical_and(exp_inds, pos_inds)
        #         next_df[-2] = next_df[-2][inds]

        #         exp_inds = next_df[-1].explicit == 1
        #         pos_inds = next_df[-1].label == 1
        #         inds = torch.logical_and(exp_inds, pos_inds)
        #         next_df[-1] = next_df[-1][inds]
        #     else:
        #         inds = next_df[-2].label == 1
        #         next_df[-2] = next_df[-2][inds]
        #         inds = next_df[-1].label == 1
        #         next_df[-1] = next_df[-1][inds]
        # else:
        #     if self.config['contaminate']:
        #         inds = next_df[-2].explicit == 1
        #         next_df[-2] = next_df[-2][inds]
        #         inds = next_df[-1].explicit == 1
        #         next_df[-1] = next_df[-1][inds]

        # make sure training is all implicit feedback
        # self.logger.info("Filtering out explicit feedback in training set...")
        # exp_inds = next_df[0].explicit == 1
        # next_df[0] = next_df[0][~exp_inds] 
        # if self.config['eval_type'] == EvaluatorType.RANKING:
        #     self.logger.info("Filtering out implicit and negative explicit feedback in validation and test set...")
        #     # we only evaluate on positive explicit dataset if it is pairwise
        #     exp_inds = next_df[-2].explicit == 1
        #     pos_inds = next_df[-2].label == 1
        #     inds = torch.logical_and(exp_inds, pos_inds)
        #     next_df[-2] = next_df[-2][inds]
        ...
        #     next_df[-1] = next_df[-1][inds]
        # else:
        #     self.logger.info("Filtering out explicit feedback in validation and test set...")
        #     # we only evaluate on implicit dataset if it is pointwise
        #     imp_inds = next_df[-2].explicit == 0
        #     next_df[-2] = next_df[-2][imp_inds]
        #     imp_inds = next_df[-1].explicit == 0
        #     next_df[-1] = next_df[-1][imp_inds]
        if self.config['eval_type'] == EvaluatorType.RANKING:
            self.logger.info("Filtering out negative explicit feedback in validation and test set...")
            # we only evaluate on positive explicit dataset if it is pairwise
            pos_inds = next_df[-2].label == 1
            next_df[-2] = next_df[-2][pos_inds]

            pos_inds = next_df[-1].label == 1
            next_df[-1] = next_df[-1][pos_inds]
    ...
    def _create_sparse_matrix(
        ...
        if form == "torch_sparse":
            indices = torch.cat([src, tgt], dim=0).reshape(2, -1)
            values = torch.tensor(data, dtype=torch.float32)
            size = (self.num(source_field), self.num(target_field))
            mat_torch = torch.sparse_coo_tensor(indices, values, size)
            return mat_torch
        else:
            mat = coo_matrix(
