...
class DCNV2(ContextRecommender):
        ...
        if config['multi_cls']:
            if self.structure == "parallel":
                self.predict_layer = nn.Linear(
                    self.in_feature_num + self.mlp_hidden_size[-1], config['multi_cls']
                )
    ...
    def calculate_loss(self, interaction):
        ...
        if self.multi_cls:
            ...
            return (1-self.multi_ratio) * (label_loss + l2_loss), rating_loss * self.multi_ratio
            # return label_loss + l2_loss, rating_loss * self.multi_ratio
        else:
