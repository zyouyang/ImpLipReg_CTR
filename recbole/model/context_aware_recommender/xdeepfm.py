...
class xDeepFM(ContextRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if config['multi_cls']:
            size_list = (
                [self.embedding_size * self.num_feature_field] + self.mlp_hidden_size + [config['multi_cls']]
            )
        else:
            size_list = (
        ...
        if config['multi_cls']:
            # self.cin_linear = nn.Linear(self.final_len, config['multi_cls'])
    ...
    def calculate_loss(self, interaction):
        ...
        if self.multi_cls:
            ...
            return (1 - self.multi_ratio) * (label_loss + self.reg_weight * l2_reg), rating_loss * self.multi_ratio
        else:
