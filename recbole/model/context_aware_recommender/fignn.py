...
class FiGNN(ContextRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if config['multi_cls']:
            ...
            for i in range(config['multi_cls']):
                self.mlp1_layers.append(nn.Linear(self.attention_size, 1, bias=False))
                self.mlp2_layers.append(
                    nn.Linear(
            ...
            self.multi_cls = True
            self.multi_cls_num = config['multi_cls']
    ...
    def fignn_layer(self, in_feature):
        ...
        if self.multi_cls:
            for i in range(self.multi_cls_num):
                score = self.mlp1_layers[i](h).squeeze(-1)
                weight = self.mlp2_layers[i](h.flatten(start_dim=1))
                logit = (weight * score).sum(dim=1).unsqueeze(-1)
                if i == 0:
                    output = logit
                else:
                    output = torch.cat([output, logit], dim=1)
            return output
        else:
            # Attentional Scoring Layer
