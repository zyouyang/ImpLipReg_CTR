...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        # grad_l2_norms = []
        # weight_l2_norms = []
        # embedding_grad_l2_norms = []
        # embedding_weight_l2_norms = []
        # not_emb_grad_l2_norms = []
        # not_emb_weight_l2_norms = []
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # print(f"forward a batch: {time() - start:.2f}s")
            ...
            # grad_l2_norm = 0.0
            # weight_l2_norm = 0.0
            # embedding_grad_l2_norm = 0.0
            # embedding_weight_l2_norm = 0.0
            # not_emb_grad_l2_norm = 0.0
            # not_emb_weight_l2_norm = 0.0
            # for name, param in self.model.named_parameters():
            #     if param.grad is not None:
            #         grad_l2_norm += param.grad.norm(2).item() ** 2
            #     if param.requires_grad:
            #         weight_l2_norm += param.norm(2).item() ** 2
            #     # Check if the parameter belongs to an embedding layer
            #     if 'embedding' in name or 'Embedding' in name:
            #         if param.grad is not None:
            #             embedding_grad_l2_norm += param.grad.norm(2).item() ** 2
            #         if param.requires_grad:
            #             embedding_weight_l2_norm += param.norm(2).item() ** 2
            #     else:
            #         if param.grad is not None:
            #             not_emb_grad_l2_norm += param.grad.norm(2).item() ** 2
            #         if param.requires_grad:
            #             not_emb_weight_l2_norm += param.norm(2).item() ** 2

            # grad_l2_norm = grad_l2_norm ** 0.5
            # weight_l2_norm = weight_l2_norm ** 0.5
            # embedding_grad_l2_norm = embedding_grad_l2_norm ** 0.5
            # embedding_weight_l2_norm = embedding_weight_l2_norm ** 0.5
            # grad_l2_norms.append(grad_l2_norm)
            # weight_l2_norms.append(weight_l2_norm)
            # embedding_grad_l2_norms.append(embedding_grad_l2_norm)
            # embedding_weight_l2_norms.append(embedding_weight_l2_norm)
            # not_emb_grad_l2_norms.append(not_emb_grad_l2_norm ** 0.5)
            # not_emb_weight_l2_norms.append(not_emb_weight_l2_norm ** 0.5)

            # print(f"backward a batch: {time() - start:.2f}s")
            ...
            # breakpoint()
        # return total_loss, grad_l2_norms, weight_l2_norms, \
        #     embedding_grad_l2_norms, embedding_weight_l2_norms, \
        #     not_emb_grad_l2_norms, not_emb_weight_l2_norms
        return total_loss
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # train_loss, grad_l2_norms, weight_l2_norms, \
            #     embedding_grad_l2_norms, embedding_weight_l2_norms, \
            #     not_emb_grad_l2_norms, not_emb_weight_l2_norms = self._train_epoch(
            #     train_data, epoch_idx, show_progress=show_progress
            # )
            ...
            if verbose:
                ...
                # self.logger.info(f"grad_l2_norm: {[round(elem, 4) for elem in grad_l2_norms]}")
                # self.logger.info(f"weight_l2_norm: {[round(elem, 4) for elem in weight_l2_norms]}")
                # self.logger.info(f"embedding_grad_l2_norm: {[round(elem, 4) for elem in embedding_grad_l2_norms]}")
                # self.logger.info(f"embedding_weight_l2_norm: {[round(elem, 4) for elem in embedding_weight_l2_norms]}")
                # self.logger.info(f"not_emb_grad_l2_norm: {[round(elem, 4) for elem in not_emb_grad_l2_norms]}")
                # self.logger.info(f"not_emb_weight_l2_norm: {[round(elem, 4) for elem in not_emb_weight_l2_norms]}")
